...
N=8 ; n=3 ; m=2  ; NT = 200 
vmin = -1 ; vmax = 1 ; wmin = -pi/3 ; wmax = pi/3
T = 0.1 
...
# Penalties 
Q= 0.5*np.eye(n); QN=200*Q; R= np.diag([0.1,0.1])


x0 = np.matrix([[0.1],[0.5],[0]]); # Initial states conditions
...
for k in range(NT) :  
    ...
    z = minimize(fun, zk, method='SLSQP' ,constraints=[nonlin_con],bounds=bnds)  # Solution
...
plt.figure(2)
plt.plot(np.transpose(x[0]) + xr_1 ,np.transpose(x[1]) + xr_2 ,'r',label='X,Y',linestyle='dashed')
xlabel("X [m]")
ylabel("Y [m]")
plt.legend()
plt.grid()
...
plt.figure(3)
plt.step(np.arange(NT), U , linewidth=1.5,label='Linear speed [m/s]')
plt.legend()
plt.grid()
plt.xlabel("Time [s]")
plt.show() 
...
plt.figure(4)
plt.step(np.arange(NT), J.T, linewidth=1.5,label='Cost Function []')
plt.legend()
plt.grid()
plt.xlabel("Time [s]")
plt.show() 
